...
public:
    ...
    HASH_INDEX_T next() 
    {
        if(this->numProbes_ >= this->m_) { // probing failed
            return this->npos;
        } 
...
private:
    ...
    double resizeAlpha_; //used to determine when to resize
    size_t numberOfItems_; //number of items in has table
    size_t numberOfNonNull_; //number of non null items in hash table
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    resizeAlpha_ = resizeAlpha; // load factor threshold
    mIndex_ =0;// smallest capacity
    numberOfItems_ =0; //number of items
    numberOfNonNull_ =0;// number of non null items
    totalProbes_ =0;// total probes

    table_.clear();// empty the table
    table_.resize(CAPACITIES[mIndex_], nullptr); // resize to original capacity
...
{
    double loadFactor = (double)numberOfNonNull_ / (double)CAPACITIES[mIndex_];// calculate load factor
    if(loadFactor >= resizeAlpha_){
        resize(); // resize if load factor greater than threshold
    }

    HASH_INDEX_T loc = probe(p.first); // get location to insert
    if(loc == npos){ // no free location found
        throw std::logic_error("No free location found");
    }

    if(table_[loc] == nullptr){ // insert new item
...
{
    size_t capacityCount = sizeof(CAPACITIES)/sizeof(HASH_INDEX_T);
    if(mIndex_ +1 >= capacityCount){ // check if capacity available
        throw std::logic_error("No more CAPACITIES exist");
    }
    std::vector<HashItem*> oldTable = table_; //keep old table

    mIndex_ = mIndex_ +1; // increase index
    table_.clear(); // clear current table
    table_.resize(CAPACITIES[mIndex_], nullptr); // resize to new capacity
    numberOfItems_ =0;
    numberOfNonNull_ =0;

    for(size_t i=0; i<oldTable.size();i++){
        HashItem* item = oldTable[i];// get old item
        if(item!= nullptr){
            if(item->deleted == false){ // rehash only not deleted items
                insert(item->item); // add to new table
            }
            delete item; 
        }
    }
